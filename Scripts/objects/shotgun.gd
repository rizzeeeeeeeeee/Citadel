...
func _shoot() -> void:
	...
	# Количество пуль и угол разброса
	var bullet_count: int = 5
	var spread_angle: float = 30.0  # Угол разброса в градусах
	...
	for i in range(bullet_count):
		...
		if bullet and muzzle:
			...
			var angle_offset: float = -spread_angle / 2.0 + (spread_angle / (bullet_count - 1)) * i
			var bullet_direction: Vector2 = -muzzle.global_transform.y.rotated(deg_to_rad(angle_offset))
